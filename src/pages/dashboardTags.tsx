...
const DashboardTags = () => {
  ...
  const handleDeleteEvent = async (id: number) => {
    try {
      ...
      if (confirm) {
        await CustomFetch.delete(`/tags/delete/${id}`);
    ...
    <>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">Jumlah Tag {tags.length}</p>
        </div>
      </div>

      {/* Table */}
      <DataTable data={tags} columns={columns} getRowKey={(item) => item.ID} />
    </>
  );
};
